...
fn main() {
	tauri::Builder::default()
		...
		.setup(|app| {
			...
			#[cfg(target_os = "macos")]
			let dock_hidden = store.get("dock_hidden").and_then(|value| value.as_bool()).unwrap_or(false);
			...
			let increase = MenuItem::with_id(app, "increase", "Increase Size\u{2001}➕", true, None::<&str>)?;

			let decrease = MenuItem::with_id(app, "decrease", "Decrease Size\u{2001}➖", true, None::<&str>)?;
			...
			#[cfg(target_os = "macos")]
			let dock = CheckMenuItem::with_id(app, "dock", "No Dock\u{2001}⚓", true, dock_hidden, None::<&str>)?;
			...
			let tray = TrayIconBuilder::new().menu(&menu).icon_as_template(true).icon(tray_icon);
			...
			tray.on_menu_event(move |app, event| {
				...
				if event.id.as_ref() == "dock" {
					let now_hidden = !store.get("dock_hidden").and_then(|value| value.as_bool()).unwrap_or(false);
				...
				let mut size = store.get("size").and_then(|value| value.as_i64()).unwrap_or(DEFAULT_SIZE);
				...
				let mut hidden = store.get("hidden").and_then(|value| value.as_bool()).unwrap_or(false);
				...
				for (_label, window) in app.webview_windows() {
					...
					let _ = window.emit("mode", Payload { message:Message::Mode(mode.clone()) });
